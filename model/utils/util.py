...
def get_interpolate_frames(frame_list_pre, frame_list_post):
    ...
    for i in range(len(frame_list_pre)):
        ...
        # interpolated_frames.append(frame_list_pre[i] * 0.5 + frame_list_post[i] * 0.5)
        # interpolated_frames.append(frame_list_pre[i] * (1-post_w_list[i]) + frame_list_post[i] * post_w_list[i])
        # frame_list_pre[i]=np.clip(frame_list_pre[i], 1e-6, None)
        # frame_list_post[i]=np.clip(frame_list_post[i], 1e-6, None)
        # interpolated_frames.append(np.exp(np.log(frame_list_pre[i]) * (1-post_w_list[i]) + np.log(frame_list_post[i]) * post_w_list[i]))
