...
class CrossAttention(nn.Module):
    ...
    def compute_pose_bias(self, pose_embeddings):
        B_L_N, T, C = pose_embeddings.shape
        
        # 计算pose embedding之间的相似度
        pose_similarity = self.compute_embedding_similarity(pose_embeddings)  # [B*L*N, T, T]
        
        return pose_similarity
    
    def compute_embedding_similarity(self, embeddings):
        """计算pose embedding之间的相似度"""
        # 使用余弦相似度或点积相似度
        embeddings_norm = F.normalize(embeddings, dim=-1)  # [B*L, T, C]
        similarity = torch.matmul(embeddings_norm, embeddings_norm.transpose(-2, -1))  # [B*L, T, T]
    ...
    def _attention(self, query, key, value, attention_mask=None):
        ...
        # attention_scores=attention_scores+pose_bias
        if attention_mask is not None:
