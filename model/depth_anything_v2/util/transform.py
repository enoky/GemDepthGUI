...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        if self.__resize_target:
            ...
            if "image_ori" in sample:
                sample["image_ori"] = cv2.resize(sample["image_ori"], (width, height), interpolation=self.__image_interpolation_method)
...
class PrepareForNet(object):
    ...
    def __call__(self, sample):
        ...
        if "flow" in sample:
            sample["flow"] = sample["flow"].astype(np.float32)
            sample["flow"] = np.ascontiguousarray(sample["flow"])
        
        if "image_ori" in sample:
            image_ori = np.transpose(sample["image_ori"], (2, 0, 1))
            sample["image_ori"] = np.ascontiguousarray(image_ori).astype(np.float32)
